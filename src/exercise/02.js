...
function Greeting({initialName = ''}) {
  ...
  // LAZY INITTIALIZER: inicialização "preguiçoso"
  // Quando o useState recebe uma função em vez de um valor como estado inicial,
  // essa função é executada apenas na fase mount do componente, sem se
  // repetir na fase update.
  const [name, setName] = React.useState(() => 
    window.localStorage.getItem('name') || initialName)
  const [count, setCount] = React.useState(0)
  const [nameUC, setNameUC] = React.useState(() => 
    window.localStorage.getItem('nameUC') || initialName)
  ...
  // quando a variável name ou nameUC sofrer alteração.
  ...
  return (
    <div>
      ...
      <p>
          localStorage: {window.localStorage.getItem('name')} - 
          {window.localStorage.getItem('nameUC')}
      </p>
      <p>Contagem: {count}</p>
    </div>
  )
